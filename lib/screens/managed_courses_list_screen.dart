...
import 'managed_course_detail_screen.dart'; // شاشة تفاصيل الدورة
import 'create_edit_course_screen.dart'; // <--- شاشة إضافة/تعديل دورة (تم إنشاؤها سابقاً)
...
class _ManagedCoursesListScreenState extends State<ManagedCoursesListScreen> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      ...
      if (_scrollController.position.pixels == _scrollController.position.maxScrollExtent &&
          !Provider.of<ManagedTrainingCourseProvider>(context, listen: false).isFetchingMore &&
          Provider.of<ManagedTrainingCourseProvider>(context, listen: false).hasMorePages) {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: const Text('هل أنت متأكد أنك تريد حذف هذه الدورة؟'), // يمكنك عرض اسم الدورة هنا
          actions: <Widget>[
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
            ...
            onPressed: courseProvider.isLoading ? null : () { // تعطيل الزر أثناء التحميل
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  ...
                  builder: (context) => const CreateEditCourseScreen(course: null), // <--- الانتقال لشاشة الإضافة
                ),
              );
            },
          ),
      ...
      body: courseProvider.isLoading && courseProvider.managedCourses.isEmpty // التحميل الأولي
          ? const Center(child: CircularProgressIndicator())
          : courseProvider.error != null // عرض الخطأ
          ? Center(child: Text('Error: ${courseProvider.error}'))
          : courseProvider.managedCourses.isEmpty // لا توجد بيانات
          ? const Center(child: Text('لم تنشر أي دورات بعد.'))
          : ListView.builder(
        controller: _scrollController, // ربط الـ ScrollController
        itemCount: courseProvider.managedCourses.length + (courseProvider.isFetchingMore ? 1 : 0), // إضافة عنصر تحميل في النهاية
          ...
          return ListTile(
            ...
            onTap: courseProvider.isLoading ? null : () { // تعطيل النقر أثناء التحميل
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ManagedCourseDetailScreen(courseId: course.courseId!), // <--- الانتقال لشاشة التفاصيل
                ),
              );
            },
          );
